...
<c:choose>
	<c:when test="${item['formato'] eq 'ARCHIVO_PX'}"><a href="${urlPxJaxi}${item['id']}">${item['nm_title']}</a></c:when>
	<c:when test="${item['formato'] eq 'ARCHIVO_PDF'}">
		<a href="${urlPxJaxiDescarga}${item['id']}">${item['nm_title']}</a>
	</c:when>
	<c:when test="${item['formato'] eq 'PUBLICACION'}"><a href="${urlPublicacionJaxi}${item['id']}">${item['nm_title']}</a></c:when>
	<c:otherwise>
		${item['nm_title']}
	</c:otherwise>
</c:choose>
...
<c:if test="${!empty item['nm_description']}">
 <div class="resultado_item">${item["nm_description"]}</div>
</c:if>
